...
test.describe('Tutor Dashboard Tests', () => {
  ...
  test('should navigate between different sections', async ({ page }) => {
    ...
    await expect(page.getByTestId('session-requests-container')).toBeVisible(); // Add a unique class to SessionRequests
      await page.getByRole('button', { name: 'Profile' }).click();
    await expect(page.getByTestId('tutor-profile-container')).toBeVisible(); // Add a unique class to TutorProfile
  });

  test('should open and close notifications', async ({ page }) => {
    const notificationButton = page.locator('button:has(i.fa-bell)');
    await notificationButton.click();
    await page.waitForSelector('.notifications-dropdown'); // Add this class
